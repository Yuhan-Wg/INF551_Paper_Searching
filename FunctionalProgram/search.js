...
firebase.initializeApp(config, 'inf551');

var database = firebase.app('inf551').database();


// This function take query as input, outputs results
// results presents in list of JSON object(each object contains metadata of one paper)
// keyword for keyword searching
// limit for faceted searching
// logic1 for searching logic among keywords, is ‘or’ or ‘and
// logic2 for searching logic among values of single facet, is always ‘or’
// logic3 for searching logic among different facets, is ‘or’ or ‘and ’
...
async function indexProducer(keyword,limit,logic1, logic2, logic3){
  ...
  function indexMerge(snapshot){
    ...
    // limitation
   // if logic == ‘and’:
   //     id_list = [ list of each query for one facet/keyword ]
   //     id_list = id_list.reduce(function(a,b){
   ...
   //        return a.concat(b)
   //      }) //find union
   //     id_list = unique(id_list) //remove duplication
    ...
    if( limit !== null){
      ...
      for (var key in limit){
        ...
        for(var i=0;i<limit[key].length;i++){
          ...
          var idx_limit_r = indexMerge(RefLimit)
          if(logic2 == 'and'){
...
async function indexToMetadata(idx){
    ...
    for (var i=0;i<idx.length;i++){
        ...
        ref = ref.val();
        ref['abstract'] = ref['text'].substring(0,300)+"...";
        results.push(ref);
    }
    return results
}
